...
export default function MessageBubble({ data }) {
    if (data.from === '7867893310' || data.from === '+17867893310') {
        return (
            <Fragment>
                <div className="d-flex flex-row-reverse">
                    <small className="text-muted">{moment(data.date).format("dddd, MMMM Do YYYY, h:mm:ss a")}</small>
                </div>
                <div className="d-flex flex-row-reverse">
        ...
        return (
            <div>
                <small className="text-muted">{moment(data.date).format("dddd, MMMM Do YYYY, h:mm:ss a")}</small>
                <div className="speech-bubble-in bg-success  p-1 mb-2 d-flex justify-content-between">
